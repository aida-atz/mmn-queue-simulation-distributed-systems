...
class Simulation:
    ...
    def run(self, max_t=float('inf')):
        ...
        while self.events:  # TODO: as long as the event queue is not empty:
            t,prio, event = heappop(self.events) # TODO: get the first event from the queue
            # job_id_prop = "id" if hasattr(event, "id") else "job_id"
            # job_id = getattr(event, job_id_prop)
            # logging.info(f"Popped {"Arrival" if job_id_prop == "id" else "Completion"} event: {job_id} with priority: {prio} at t: {t}")
            if t > max_t:
                break
            self.t = t
            event.process(self)
